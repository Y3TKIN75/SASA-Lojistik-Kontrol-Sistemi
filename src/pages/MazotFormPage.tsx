...
export default function MazotFormPage() {
  ...
  const isTraktor = session.role === 'traktor';
  const homePath = isTraktor ? '/home/traktor' : '/home/forklift';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { error: dbError } = await supabase.from('mazot_submissions').insert({
      ...
      forklift_no: isTraktor ? null : forkliftNo,
      calisma_saati: isTraktor ? null : calismaSaati,
      kilometre: isTraktor ? kilometre : null,
      litre,
    });
        ...
        <div className="bg-white rounded-2xl shadow-lg p-8 max-w-sm w-full text-center space-y-4">
          ...
          <div className="flex gap-3">
            <button
              onClick={() => {
                setSubmitted(false);
                setForkliftNo(''); setCalismaSaati(''); setKilometre(''); setLitre('');
              }}
      ...
      <div className="flex-1 px-4 py-4 space-y-4 max-w-lg mx-auto w-full">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 space-y-4">
            ...
            {isTraktor ? (
              <div>
                <label className="text-sm text-gray-500 block mb-1">Traktör Kilometresi</label>
                <input
                  type="number"
                  inputMode="numeric"
                  value={kilometre}
